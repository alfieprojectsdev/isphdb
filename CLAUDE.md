...
Table `traceroute_hops`: `id`, `timestamp` (DATETIME, indexed), `hop_index` (INTEGER), `hop_ip` (TEXT), `latency_ms` (REAL). The prober captures the full hop path to `1.1.1.1` every `TRACEROUTE_CYCLES` (10, ~5 min), storing one row per geolocatable (public) hop; rows sharing a `timestamp` form one capture. Only public hops are persisted — LAN/CGNAT hops are excluded via `is_mappable_hop`. The dashboard reads the most recent capture for the Route Ladder. Pruned on the same 30-day retention as `network_metrics`.

### Route Ladder
Below the full-width latency chart the dashboard renders a landscape "Route Ladder" — an ordered home→internet path, one rung per network layer, colored by tier (LAN blue / ISP purple / external teal) with each rung's live latency. It is pure SSR (no client JS, no map): the private **Local Router** and **ISP Gateway** rungs reuse the monitored targets' latest latency (they need no coordinates); the middle rungs are the captured public traceroute hops labeled/tiered via the curated `frontend/src/lib/hop-geo.json`; the last rung is the destination `1.1.1.1`. Tier colors and latency heat come from `frontend/src/lib/geo.mjs` (`tierColor`, `latencyToColor`, `lookupHopGeo`). `backend/geo_seed.py` is a dev-only script to refresh the label/tier table — the daemon never calls it. The chart also draws a red sentinel markLine at 500ms ("Offline") when any target is failing.
